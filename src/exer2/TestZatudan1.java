...
public class TestZatudan1 {
	...
	generateAskResponse(input, candidateList);
	...
	void generateAskResponse(String input, List<ResponseCandidate> candidateList) {
		double score = 0.0;
		ResponseCandidate cdd = new ResponseCandidate();
		StringBuilder sb = new StringBuilder();
		try {
			Process ps = Runtime.getRuntime().exec("mecab");
			BufferedReader br = new BufferedReader(new InputStreamReader(ps.getInputStream()));
			PrintWriter pw = new PrintWriter(new BufferedWriter(new OutputStreamWriter(ps.getOutputStream())));
			pw.println(input);
			pw.flush();
			String line;
			while ((line = br.readLine()) != null) {
				...
				if (line.split("\t")[1].split(",")[0].equals("名詞")) {
